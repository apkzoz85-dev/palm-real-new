...
function trackFormLead() {
  if (typeof window !== "undefined" && window.gtag) {
    window.gtag("event", "conversion", {
      send_to: "AW-18318896999/czkSCK7aldQcEOfekJ9E",
      value: 1.0,
      currency: "EGP",
    });
  }
...
function trackWhatsApp() {
  if (typeof window !== "undefined" && window.gtag) {
    window.gtag("event", "conversion", {
      send_to: "AW-18318896999/Uk5kCLHaldQcEOfekJ9E",
      value: 1.0,
      currency: "EGP",
    });
  }
...
function trackCall() {
  if (typeof window !== "undefined" && window.gtag) {
    window.gtag("event", "conversion", {
      send_to: "AW-18318896999/E4ueCLTaldQcEOfekJ9E",
    });
  }
}
...
export default function Home() {
  ...
  async function submitForm(ref: React.RefObject<HTMLFormElement | null>, setStatus: (s: "idle"|"sending"|"sent"|"error") => void) {
    ...
    try {
      ...
      if (data.success) { setStatus("sent"); ref.current.reset(); trackFormLead(); } else throw new Error();
          ...
          <div className="units-grid">
            {filtered.map((u, i) => (
              <div key={i} className="unit-card"><div className="unit-card-body">
                ...
                <a href={WA_URL} target="_blank" rel="noopener" onClick={trackWhatsApp} className="btn-unit" style={{ display:"block", textAlign:"center", textDecoration:"none" }}>استفسر الآن</a>
              </div></div>
            ))}
          </div>
          <p className="units-note">الأسعار استرشادية وقابلة للتغيير — يُرجى التواصل معنا للحصول على آخر قائمة أسعار رسمية قبل الحجز</p>
          <div style={{ marginTop: 28 }}><a className="btn-green" href={WA_URL} target="_blank" rel="noopener" onClick={trackWhatsApp}>💬 تحدث مع مستشارينا على واتساب</a></div>
        </div>
      </section>
        ...
        <div className="section-inner animate-in" style={{ textAlign: "center" }}>
          ...
          <div className="payment-grid" style={{ textAlign: "right" }}>
            <div className="payment-card">
              ...
              <div style={{ marginTop: 20 }}><a className="btn-primary" href={WA_URL} target="_blank" rel="noopener" onClick={trackWhatsApp} style={{ width:"100%", justifyContent:"center" }}>اطلب تفاصيل السداد</a></div>
            </div>
            <div className="payment-card">
...
>>>>>>> 78ba3008a2fc000dfd1a3c94eb64472bf0c0f5a8
      ...
      <section className="section contact" id="contact">
        <div className="section-inner animate-in" style={{ textAlign: "center" }}>
          ...
          <form className="contact-form" ref={formRef} onSubmit={(e: FormEvent) => { e.preventDefault(); submitForm(formRef, setFormStatus); }} style={{ textAlign: "right" }}>
            ...
            {formStatus === "error" && <p style={{ color: "#ef4444", fontSize: 13, textAlign: "center", marginTop: 10 }}>حدث خطأ — <a href={WA_URL} target="_blank" rel="noopener" onClick={trackWhatsApp} style={{ color: "var(--color-gold)" }}>تواصل واتساب</a></p>}
          </form>
      ...
      <div className={`popup-dialog ${showPopup?"open":""}`} role="dialog">
        ...
        {popupStatus === "sent" ? (
          ...
          <form className="popup-form" ref={popupFormRef} onSubmit={(e: FormEvent) => { e.preventDefault(); submitForm(popupFormRef, setPopupStatus).then(() => { if (popupFormRef.current) setTimeout(closePopup, 2500); }); }}>
            ...
            <a className="popup-wa-link" href={WA_URL} target="_blank" rel="noopener" onClick={trackWhatsApp}>💬 تواصل واتساب</a>
          </form>
        )}
      </div>
      ...
      <nav className="mobile-bar">
        <div className="mobile-bar-inner">
          ...
          <a className="bar-wa" href={WA_URL} target="_blank" rel="noopener" onClick={trackWhatsApp}>💬 واتساب</a>
